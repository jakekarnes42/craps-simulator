        ...
        <>
            ...
            <div className='mb-0'>
                <ButtonGroup id={`${id}-buttongroup`}>
                    ...
                    <Form.Check.Label bsPrefix='btn btn-outline-primary' onClick={() => { if (!disabled) onChange(RoundingType.DOLLAR) }}>{RoundingType.DOLLAR}</Form.Check.Label>
                    <Form.Check.Input type='radio' bsPrefix='btn-check' disabled={disabled} checked={rounding === RoundingType.CENT} readOnly />
                    <Form.Check.Label bsPrefix='btn btn-outline-primary' onClick={() => { if (!disabled) onChange(RoundingType.CENT) }}>{RoundingType.CENT}</Form.Check.Label>
                </ButtonGroup>
            </div>
            <div className='mb-0'>
